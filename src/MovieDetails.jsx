...
const MovieDetails = () => {
    ...
    const getMovieDetails = async () => {
      try {
        ...
        const charactersData = await Promise.all(
          response.data.characters.map((character) => axios.get(character))
        );
        const charactersNames = charactersData.map(
          (character) => character.data.name
        );
        setCharacterList(charactersNames);

         const planetsData = await Promise.all(
           response.data.planets.map((planet) => axios.get(planet))
         );
         const planetsNames = planetsData.map(
           (planet) => planet.data.name
         );
         setPlanetList(planetsNames);

          const speciesData = await Promise.all(
            response.data.species.map((specie) => axios.get(specie))
          );
          const speciesNames = speciesData.map(
            (specie) => specie.data.name
          );
          setSpeciesList(speciesNames);

           const starshipsData = await Promise.all(
             response.data.starships.map((starship) => axios.get(starship))
           );
           const starshipsNames = starshipsData.map(
             (starship) => starship.data.name
           );
           setStarshipsList(starshipsNames);

            const vehicleData = await Promise.all(
              response.data.vehicles.map((vehicle) => axios.get(vehicle))
            );
            const vehicleNames = vehicleData.map(
              (vehicle) => vehicle.data.name
            );
            setVehiclesList(vehicleNames);
  ...
  const { title, director, producer, opening_crawl } = details;
